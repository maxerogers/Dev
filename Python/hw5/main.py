...
class Assign(Node):
    ...
    def execute(self):
        ...
        if(type(self.left) == Index): #This catches leftside-Index Node assignments
            array = self.left.location()
            index = self.left.right.evaluate()
            array[index] = self.right.evaluate()
        else: ##This is for when the left side is a Variable Node
            vars[self.left.location()] = self.right.evaluate()
